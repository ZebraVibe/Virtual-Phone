 ...
 *
 */
public class FileTest extends ApplicationAdapter{
	...
	private static final Logger log = new Logger(FileTest.class.getName(), Logger.DEBUG);
...
//				String toBeSentMsg = msg.getText().length() == 0 ? "Hello from Client\n" : msg.getText() + "\n"; // !! "+\n"
				File toBeDownloadedFile = new File("C:\\Users\\sebas\\Documents\\Eclipse\\libgdx\\lwjgl3test workspace\\"
...
//				DataOutput out = new DataOutput(new BufferedOutputStream(clientSocket.getOutputStream()));
...
//					JarFile jar = new JarFile(toBeDownloadedFile);
//					Manifest man = jar.getManifest();
//					JarOutputStream jarOut = new JarOutputStream(out);
//					long byteCount = Files.copy(toBeDownloadedFile.toPath(), jarOut); // copies all bytes from the file to thge outputstream
//					jarOut.close(); // !!!!! else when reading the outputstream never -1 is returned
					
					long byteCount = Files.copy(toBeDownloadedFile.toPath(), out); // copies all bytes from the file to thge outputstream
					out.close(); // !!!!! else when reading the outputstream never -1 is returned


					
					long end = System.currentTimeMillis() - start;
					log.debug("[Client]...done! "+ end + " ms to send " + byteCount + " bytes!");
...
//					BufferedReader buffer = new BufferedReader(new InputStreamReader(socket.getInputStream()));
					BufferedInputStream in = new BufferedInputStream(socket.getInputStream()); // reads many bites at a time
